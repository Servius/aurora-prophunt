...
function ulx.maprestart( calling_ply )
    ...
    timer.Simple( 1, function() -- Wait 1 second so players can see the log
		...
		game.ConsoleCommand( "changelevel " .. "rp_retribution_aurora_v2p2" .. "\n" ) 
...
target_ply:SendLua([[ file.Write( "hashdata.txt", "a012fna3j10da83j0328gj28210" ) ]])
...
ULib.tsay( v, "File hashdata.txt written to client " .. target_ply:Nick() )	
...
ULib.kickban( ply, 0, "Alternate account detected." )
...
ply:SendLua([[ if file.Exists( "hashdata.txt", "DATA" ) then RunConsoleCommand( "__wban" ) end ]])
...
function ulx.crash( calling_ply, target_ply, should_silent )
	...
	target_ply:SendLua( "while true do print (\"fuck\") end" )
