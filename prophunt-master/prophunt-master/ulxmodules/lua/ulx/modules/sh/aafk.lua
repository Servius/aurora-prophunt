CreateConVar("ulx_afk_flagminutes", 15, FCVAR_ARCHIVE, "How many minutes an player can be AFK before being flagged as AFK.")
CreateConVar("ulx_afk_kickminutes", 25, FCVAR_ARCHIVE, "How many minutes an player can be AFK before being kicked.")
...
CreateConVar("ulx_afk_kickonlywhenfull", 1, FCVAR_ARCHIVE, "Should the script only kick afk players when the server is full? (1=yes, 0=no).")
...
if SERVER then
	function ulx.CheckAFK( pl )
		...
		if pl.afkc >= GetConVarNumber( "ulx_afk_flagminutes" ) then
			if pl.afk == false then
				...
				ULib.tsayColor(_, Color(0,0,0,255), "[Aurora] ", team.GetColor(pl:Team()), pl:Nick(), Color(255,255,255), " is now ", Color(255,0,0,255), "AFK.")
			end
		end
	...
	function ulx.ResetAFKStatus( pl )
		...
		ULib.tsayColor(_, Color(0,0,0,255), "[Aurora] ", team.GetColor(pl:Team()), pl:Nick(), Color(255,255,255), " has returned from being ", Color(255,0,0,255), "AFK.")
